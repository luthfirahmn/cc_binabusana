...
function create_code($module)
{
    $_ci = &get_instance();
    switch ($module) {
        case 'asset_type':
            $_ci->db->select('RIGHT(ms_asset_type.asset_type_code,5) as code', FALSE);
            $_ci->db->order_by('asset_type_code', 'DESC');
            $_ci->db->limit(1);
            $query = $_ci->db->get('ms_asset_type');
            if ($query->num_rows() <> 0) {
                $data = $query->row();
        ...
        case 'asset':
            $_ci->db->select('RIGHT(ms_asset.asset_code,5) as code', FALSE);
            $_ci->db->order_by('asset_code', 'DESC');
            $_ci->db->limit(1);
            $query = $_ci->db->get('ms_asset');
            if ($query->num_rows() <> 0) {
                $data = $query->row();
                $kode = intval($data->code) + 1;
            } else {
            ...
            $batas = str_pad($kode, 5, "0", STR_PAD_LEFT);
            $kodetampil = "ASE" . $batas;
            return $kodetampil;
            break;

        case 'area':
            $_ci->db->select('RIGHT(ms_area.area_code,5) as code', FALSE);
            $_ci->db->order_by('area_code', 'DESC');
            $_ci->db->limit(1);
            $query = $_ci->db->get('ms_area');
            if ($query->num_rows() <> 0) {
                $data = $query->row();
                $kode = intval($data->code) + 1;
            } else {
        ...
        case 'customer':
            $_ci->db->select('RIGHT(ms_customer.customer_code,5) as code', FALSE);
            $_ci->db->order_by('customer_code', 'DESC');
            $_ci->db->limit(1);
            $query = $_ci->db->get('ms_customer');
            if ($query->num_rows() <> 0) {
                $data = $query->row();
                $kode = intval($data->code) + 1;
            } else {
        ...
        case 'booking':
            $_ci->db->select('RIGHT(tr_booking.trx_code,5) as code', FALSE);
            $_ci->db->order_by('trx_code', 'DESC');
            $_ci->db->limit(1);
            $query = $_ci->db->get('tr_booking');
            if ($query->num_rows() <> 0) {
                $data = $query->row();
                $kode = intval($data->code) + 1;
            } else {
